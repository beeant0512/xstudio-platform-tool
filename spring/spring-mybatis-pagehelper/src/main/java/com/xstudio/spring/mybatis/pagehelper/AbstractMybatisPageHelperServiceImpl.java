...
public abstract class AbstractMybatisPageHelperServiceImpl<T extends BaseModelObject, K> extends AbstractServiceImpl<T, K, PageBounds, PageList<T>, List<T>> {
    ...
    @Override
    public Msg<PageList<T>> selectAllByExampleWithBlobs(T record, List<?> orders) {
        Msg<PageList<T>> msg = new Msg<>();
        Msg<PageList<T>> pageListMsg = new Msg<>();
        List<T> list = new ArrayList<>();
        Paginator paginator = new Paginator(1, 1, 1);
    ...
    public Msg<PageList<T>> selectByExampleByPager(T record, PageBounds pageBounds) {
        ...
        List<T> result = getRepositoryDao().selectByExample(record, pageBounds);
        if (result.isEmpty()) {
            msg.setResult(EnError.NO_MATCH);
            return msg;
        }
    ...
    public Msg<PageList<T>> selectByExampleWithBlobsByPager(T record, PageBounds pageBounds) {
        ...
        List<T> result = getRepositoryDao().selectByExampleWithBLOBs(record, pageBounds);
        if (result.isEmpty()) {
            msg.setResult(EnError.NO_MATCH);
            return msg;
        }
